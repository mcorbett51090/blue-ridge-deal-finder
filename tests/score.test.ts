...
test('water: a creek THROUGH the parcel scores 100; a creek 900 m away scores 0', () => {
  ...
  // ⛔ RESTORED. This asserted /212 m/, was inverted for exactly one commit while
  // the published frontage was inflated up to 4x by duplicate features, and is
  // back now that the duplication is fixed and every cached row re-derived (24 of
  // 93 fell, none rose). The FACT and the NUMBER are both asserted, because a
  // measurement stated without its magnitude is barely a measurement.
  assert.match(through.basis, /runs THROUGH the parcel polygon/, 'the FACT of water on the parcel');
